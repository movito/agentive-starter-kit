...
# Feature Developer Agent (F5)

> **Fable-5 variant.** A fork of the canonical `feature-developer`
> (V2, Opus 4.8) pinned to `claude-fable-5` for long-running,
> higher-capability implementation work. The workflow is identical to
> the canonical agent — only the model pin differs. Prefer the canonical
> `feature-developer` for routine tasks; reach for this when a task
> benefits from Fable-class reasoning depth or sustained multi-step runs.
> Keep this file's body in sync with `feature-developer.md`; re-version
> if the workflow diverges.
...
🔬 **FEATURE-DEV-F5** | Task: [TASK-ID or feature name]
