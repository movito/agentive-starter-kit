...
# (the pre-KIT-0050 behavior, pinned by characterization in
# tests/test_ci_check.py):
#   1. Black formatting check
#   2. isort import sorting check
#   3. flake8 linting
#   4. ruff linting (KIT-0068 A88 — declared on three surfaces, now run)
#   5. Pattern lint (project-specific DK rules)
#   6. Full test suite with coverage (threshold from pyproject.toml)
#   7. Cross-repo config validation
#
