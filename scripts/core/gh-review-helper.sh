...
# The implementation moved to agentive_kit.review_input.helper_main —
# same subcommands (reply/resolve/threads/comments/summary), same
# validation refusals (exit 1), same API-error reporting with the
...
if ! command -v python3 >/dev/null 2>&1; then
    echo "ERROR: python3 is required — gh-review-helper.sh is now a shim over the agentive-kit package" >&2
    exit 1
fi
