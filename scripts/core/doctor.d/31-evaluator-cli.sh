...
if [ "${DOCTOR_EVALUATORS:-}" = "no" ]; then
    echo "DOCTOR:evaluator-cli:SKIP:evaluators declined at install (evaluators: no in the kit-install record) — install them with 'agentive install-evaluators' and update the record to re-enable this check"
    exit 0
fi
