#!/usr/bin/env bash
# engine-export.sh — the clean-export ENGINE behind the setup door
# (KIT-0053, ADR-0027 P3). Formerly the scripts/optional/create-project.sh
# entrance; that filename is now a shim onto scripts/local/bootstrap
# (--new). Call the door, not this engine, unless you are the door.
#
# Usage (door-internal): engine-export.sh <target-dir> [--name NAME] [--prefix PREFIX]
#
# This script:
...
# pre-rename export result.
NAME_PLACEHOLDER='name = "your-project-name"  # TODO: Change this to your project name'
sed -i '' "s/^name = .*/$NAME_PLACEHOLDER/" pyproject.toml 2>/dev/null || \
sed -i "s/^name = .*/$NAME_PLACEHOLDER/" pyproject.toml
...
echo "  6. Start planning: claude --agent .claude/agents/planner.md"
echo ""
