...
- **o3 calibration, updated: streaks are priors, not verdicts.** Its FAIL went 3-for-3 on real blockers (KIT-0046/0048/0049), then 0-for-4 on KIT-0050 — every claim empirically wrong, and its suggested fix would have introduced the bug it alleged. Treat an o3 FAIL as worth full triage attention, never as pre-validated; its test-gap findings retained value even in the fully-refuted round. (KIT-0046→0050)
- **A retro is testimony; installable reality is checkable.** Second in-retro misattribution of the arc: KIT-0050 asserted `ADVERSARIAL_UNATTENDED` "now exists and is required" and corrected memory accordingly — planner re-verification found zero matches in the venv package, system package, AND evaluator scripts, with 1.0.1 the latest release anywhere. The phantom flag from self-review item 10 got re-asserted as newly-real, most likely by our own docs echoing through session context. Before acting on any retro claim about an external tool's interface, grep the installed artifact — the same rule specs already follow. (KIT-0050, planner verification)
