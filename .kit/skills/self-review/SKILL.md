...
8. **Wrapper exit codes (frozen contracts)**: a wrapper around an engine with a frozen exit-code contract must NOT reuse the engine's reserved success/drift codes (`0`/`1`) for wrapper-level failures. Environment/precondition failures (can't fetch, can't branch, engine missing) exit `2` — automation reading the exit code must never mistake a hard failure for "updates available". Contract: `scripts/core/sync_from_manifest.py` module docstring; compliant wrapper: `cmd_sync` in `scripts/core/project`. Three separate KIT-0036 bot findings traced to this one principle.
...
| **Wrapper exit codes** | Wrapper failures never reuse an engine's reserved `0`/`1`; precondition failures exit `2` | Automation misreading a hard wrapper failure as engine success/drift |
