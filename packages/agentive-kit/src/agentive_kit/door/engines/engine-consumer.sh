...
#   --preserve-regions
#              (door-internal, KIT-0104) modifies --internal-record-only
#              for ADOPT targets: existing KIT-LOCAL regions are
#              PRESERVED (append-if-absent), never reseeded. The reseed
#              semantics above assume a fresh export whose regions are
#              the kit's own; an adopted repo's regions are
#              consumer-owned, so the packaged door's adopt path passes
#              this flag. No effect without --internal-record-only
#              (the non-record path already preserves).
...
rm -f "$TARGET/tests/test_kit_markers.py" \
      ...
      "$TARGET/tests/test_door_data_sync.py" \
...
# (the consumer-owned KIT-LOCAL semantics stay untouched). The packaged
# door's adopt path (KIT-0104) passes --preserve-regions: an adopted
# repo's regions are consumer-owned, so reseed is disabled there.
seed_region() {
    if [ "$RECORD_ONLY" -eq 1 ] && [ "$PRESERVE_REGIONS" -eq 0 ] &&
       printf '%s\n' "$REGIONS_OUT" | grep -qx "$1"; then
