...
def _parse_main_args(argv: list[str]) -> tuple[str, str, str]:
    ...
    while i < len(argv):
        ...
        if arg in ("--help", "-h"):
            ...
            _err("Run: ./scripts/core/prepare-review-input.sh --help")
            sys.exit(1)
        else:
            if not task_id:
                task_id = arg
...
def main(argv: list[str] | None = None) -> None:
    ...
    parts.append("> Generated by `scripts/core/prepare-review-input.sh`.\n")
    parts.append(
...
Examples:
  ./scripts/core/gh-review-helper.sh summary 53
  ./scripts/core/gh-review-helper.sh --repo movito/ixda-services threads 53
  ./scripts/core/gh-review-helper.sh reply 53 2861292837 \
"""
    + """'Fixed in abc1234: description.'
  ./scripts/core/gh-review-helper.sh resolve PRRT_kwDORNcO0s5wPovc"""
)
...
def _api_error(repo: str, subject: str, stderr_content: str, context: str) -> None:
    ...
    if stderr_content:
        if re.search(
            ...
            _err(
                "        Example: ./scripts/core/gh-review-helper.sh "
